...
import { useAccount, useDisconnect, useEnsAvatar, useEnsName, useReadContract, useSwitchChain } from "wagmi";
...
import { Button, Modal, Avatar, Popover, Menu, Dropdown, Space, Progress, Spin } from "antd";
import { ExportOutlined, CaretRightOutlined, LeftOutlined, CaretDownOutlined } from "@ant-design/icons";
...
export default function AirdropPage() {
	...
	const [currentState, setCurrentState] = useState<"index" | "flap" | "leaderboard" | "partnership">("index");
	...
	const [allocationFinishedModal, setAllocationFinishedModal] = useState(false);
	...
	const { data: partnersData } = useSWR(`/api/partner-lists?sort=order:asc`, fetcherStrapi);
	...
	const { data: totalPartnershipData, mutate: totalPartnershipMutate } = useSWR(`/api/partnerships`, fetcherStrapi);

	const { data: allocationsData } = useSWR(`/api/sumAllAllocations`, fetcherStrapi);
	...
	const currentSelectedProject = partnersResult?.find(
		(data: any) => data?.attributes?.project_name?.toLowerCase() === dropdownValue.toLowerCase()
	);

	const currentSelectedContract = useReadContract({
		abi: currentSelectedProject?.attributes?.abi ?? [],
		// @ts-ignore
		address: currentSelectedProject?.attributes?.contract_address ?? "0x000",
	...
	const currentSelectedContract2 = useReadContract({
		...
		// @ts-ignore
		address: currentSelectedProject?.attributes?.contract_address2 ?? "0x000",
		functionName: "balanceOf",
		args: [address ?? ""],
	});
	...
	const addressNotMatch = address !== currentTwitterData?.attributes?.wallet_address;
	...
	const isEligible = !address
		...
		? false
		: parseFloat(tokenData1) >= (currentSelectedProject?.attributes?.min_value ?? 0) ||
		  parseFloat(tokenData2) >= (currentSelectedProject?.attributes?.min_value2 ?? 0);
		...
		axiosApi
			.post("/api/partnerships", {
				data: {
					...
					allocation: currentSelectedProject?.attributes?.allocation ?? 0,
					value1: currentSelectedProject?.attributes?.isNft
						? parseInt(currentSelectedContract?.data?.toString() ?? "0")
						: parseFloat(tokenData1 ?? "0"),
					value2: currentSelectedProject?.attributes?.isNft
						? parseInt(currentSelectedContract2?.data?.toString() ?? "0")
	...
	const handlePartnershipButton = () => {
		if (!session && !currentTwitterData?.attributes?.["is_socialaction"]) return;
		if (partnershipResult) {
			setAllocationDoneModal(true);
		} else {
			if (allocationsResult && allocationsResult?.totalAllocations >= allocationsResult?.maxAllocations) {
	...
	useEffect(() => {
		if (currentTwitterData?.attributes?.wallet_address) {
			Cookie.set("wallet_address", currentTwitterData?.attributes?.wallet_address as string, {
				expires: 1,
			});
		} else if (address) {
			...
			>
						...
						<div>
							<div className="flex flex-col items-center md:items-start gap-y-[20px]">
								...
								<div className="md:text-left text-center font-bold md:text-[16px] text-[12px]">
									<p>3. top 100 players on flappyblast will get extra allocation</p>
									<p className="md:ml-8 ml-4 text-left">-Tier 1 alloc: Top 1 Player</p>
									<p className="md:ml-8 ml-4 text-left">-Tier 2 alloc: Top 10 Player</p>
									<p className="md:ml-8 ml-4 text-left">-Tier 3 alloc: Top 100 Player</p>
									<p className="md:ml-8 ml-4 text-left">-Tier 4 alloc: Top 1000 Player</p>
								</div>
							...
							<div className="flex md:flex-row flex-col gap-x-[40px] justify-center items-center">
								...
								>
									<Image
										...
										height={100}
										alt="button"
										src={`/images/${session ? "flap_button.png" : "Flap_Disabled.png"}`}
									/>
								</div>
								<div
									onClick={() => {
								...
								<div
									...
									className={`md:block hidden relative mt-[25px] ${
										session && currentTwitterData?.attributes?.["is_socialaction"]
											? "cursor-pointer"
											: "cursor-not-allowed"
									}`}
								>
									...
									<tbody className="overflow-x-auto">
										{leaderboardsResult &&
											leaderboardsResult?.map(
												(sortedLeaderboardData: DataObject, index: number) => (
													<tr
														key={index}
													...
													>
														...
														<td className="py-4 px-2 whitespace-nowrap hidden md:table-cell">
															<div className="flex justify-center">
																<img
																	...
																	src={
																		sortedLeaderboardData?.attributes
																			?.twitter_pic ?? ""
																	}
																	alt="avatar"
																/>
														...
														<td className="py-4 px-2 whitespace-nowrap text-sm md:text-base">
															{/* @ts-ignore */}@
															{sortedLeaderboardData?.attributes?.twitter_username}
														</td>
														<td className="py-4 px-2 whitespace-nowrap text-sm md:text-base">
															{sortedLeaderboardData?.attributes?.high_score}
														</td>
													</tr>
												)
											)}
				...
				>
					<div className="flex flex-col gap-2">
						<div className="text-center flex flex-col gap-6">
							<div>
								<p>
									Select the project you belong to, but remember, you can only link with one project.
									Ensure you&apos;ve met all the requirements to successfully claim your rewards.
								</p>
							</div>
						</div>
						{!address ? (
								...
								*wrong wallet connected: connect to{" "}
								<span className="font-bold">
									...
									{currentTwitterData?.attributes?.wallet_address?.slice(
										33,
										currentTwitterData?.attributes?.wallet_address?.length
									)}
								</span>
							</p>
						)}
						{dropdownValue !== "Choose Project" && (
							<div className="flex gap-x-[10px] items-center justify-center roboto">
								...
								<div
									className={`${isEligible ? "bg-[#4FB768]" : "bg-[#B74F4F]"} rounded-[3px] p-[8px]`}
								>
									<p className="text-white">{isEligible ? "Eligible" : "Not eligible"}</p>
								</div>
							</div>
						)}
						<Dropdown
						...
						>
							<a
								className={`${
									dropdownValue === "Choose Project" ? "text-[#878787]" : "text-[#000000]"
						...
						<Progress
							...
							strokeColor="#4FB768"
							status="success"
							percent={(allocationsResult?.totalAllocations / allocationsResult?.maxAllocations) * 100}
							showInfo={false}
						/>
						<p className="font-bold text-center">
							{allocationsResult?.totalAllocations?.toLocaleString("en-US")} /{" "}
							{allocationsResult?.maxAllocations?.toLocaleString("en-US")} Claimed
						</p>
						<div className="flex justify-center w-full mt-[10px]">
							{partnershipLoading ? (
								<Spin className="md:block hidden" size="large" />
							) : (
								<div
									onClick={handleCheckPartnership}
									className={`md:block hidden relative ${
										enableCheckBtn && isEligible ? "cursor-pointer" : "cursor-not-allowed"
									}`}
								>
									<Image
										width={300}
										...
										alt="button"
										src={`/images/${
											enableCheckBtn && isEligible ? "Check_Btn.png" : "Check_Now_Disabled.png"
										}`}
									/>
								</div>
							)}
							...
							{partnershipLoading ? (
								<Spin className="block md:hidden" size="large" />
							) : (
								<div
									onClick={handleCheckPartnership}
				...
				>
					<div className="flex flex-col gap-2">
						<div className="text-center flex flex-col gap-6">
							<p>
								Congratulations! Your wallet is now successfully connected to the{" "}
								{partnershipResult?.attributes?.partner_list?.data?.attributes?.project_name ??
									currentSelectedProject?.attributes?.project_name}{" "}
								project. Just a heads up, you can only claim the extra allocation once !
							</p>
						</div>
						<a
					...
					<div className="flex flex-col gap-2">
						<div className="text-center flex flex-col gap-6">
							<p>
								...
								{partnershipResult?.attributes?.partner_list?.data?.attributes?.project_name ??
									currentSelectedProject?.attributes?.project_name}{" "}
								project. Remember, you can only connect with one project at a time.
							</p>
						</div>
						<a
